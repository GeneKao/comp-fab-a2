...
TestResult testFixedJoint() {
	...
	for (int i = 0; i < x.size(); ++i) {
		...
		VectorXd cp = joint.computeConstraints(xp, rb);
		VectorXd cm = joint.computeConstraints(xm, rb);
		dC_dx_FD.block<2, 1>(0, i) = (cp-cm) / (2*dx);
	}
...
TestResult testRigidBodySim() {
	...
	MatrixXd dCdx = sim.energy.computeJacobian(x);
	...
	for (int i = 0; i < x.size(); ++i) {
		...
		VectorXd cp = sim.energy.computeConstraints(xp);
		VectorXd cm = sim.energy.computeConstraints(xm);
...
int main(int, char**)
{
	test_function(testRigidBody, "rigid body test");
	test_function(testHingeJoint, "hinge joint test");
	test_function(testFixedJoint, "fixed joint test");
	test_function(testFixedAngleJoint, "fixed angle joint test");
	test_function(testRigidBodySim, "simulation test");
